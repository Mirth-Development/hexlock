...
use crate::features::game_controller::systems::{charge_charge_bar, check_game_state, check_tumbler_order, enter_arrow_code, handle_game_state, handle_lose_game, increase_tumbler_amount_per_level, spawn_charge_bar, spawn_lock_order};
...
impl Plugin for SystemsForUserInterfaceStates {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(OnEnter(Interfaces::Level2), (setup_level_2, load_lock_resources, increase_tumbler_amount_per_level.run_if(run_once), spawn_lock, spawn_lockpick, spawn_charge_bar,spawn_lock_order).chain());
        app.add_systems(OnExit(Interfaces::Level2), (record_level_2_exit, cleanup_entities).chain());

        app.add_systems(OnEnter(Interfaces::Level3), (setup_level_3, load_lock_resources, increase_tumbler_amount_per_level.run_if(run_once), spawn_lock, spawn_lockpick, spawn_charge_bar,spawn_lock_order).chain());
        app.add_systems(OnExit(Interfaces::Level3), (record_level_3_exit, cleanup_entities).chain());

        app.add_systems(OnEnter(Interfaces::Level4), (setup_level_4, load_lock_resources, increase_tumbler_amount_per_level.run_if(run_once), spawn_lock, spawn_lockpick, spawn_charge_bar,spawn_lock_order).chain());
        app.add_systems(OnExit(Interfaces::Level4), (record_level_4_exit, cleanup_entities).chain());

        app.add_systems(OnEnter(Interfaces::Level5), (setup_level_5, load_lock_resources, increase_tumbler_amount_per_level.run_if(run_once), spawn_lock, spawn_lockpick, spawn_charge_bar,spawn_lock_order).chain());
