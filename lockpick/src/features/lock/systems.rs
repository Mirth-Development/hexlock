        ...
        for x in 1..= lock.num_of_tumblers {
            ...
            if x == 1 {
                tumbler = parent_node.spawn((
                    gen_random_tumbler(x, tumbler_set_timer.clone(), &mut rng.random_number_generator, &lock_sprite_handles,),
...
pub fn handle_catching_tumblers (
    ...
    let Ok((focused_entity, focused_tumbler_transform, mut focused_tumbler)) = tumbler_query.single_mut() else {return};
